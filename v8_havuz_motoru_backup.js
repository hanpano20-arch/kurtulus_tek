    ...
    window.HavuzMotoru = {
      ...
      extractDetailsForUI: function (n, df, joks = []) {
        ...
        if (is_in_last_10 && (is_komsu_1 || is_joker_komsu)) {
            k15 = (config.PUAN_KOMBINE_ISINMA || 75);
        }
        ...
        let k17 = 0;
        if (is_komsu_1 && kuraklik_haftasi >= 15) {
            k17 = (config.PUAN_CAPRAZ_KURAKLIK || 60);
        }
        ...
        if (temp_is_in_last) {
            ...
            if (streak_events >= 3) {
                k18 = (config.PUAN_PATLAMAYA_HAZIR || 75);
            } else if (streak_events <= 1) {
                k18 = -(config.CEZA_PATLAMAYA_HAZIR || 100);
            }
        }
      ...
      autoTune: async function () {
        ...
        if (best_config) {
          ...
          const idMap = {
            PUAN_1_HALKA_KOMSU: 'komsu', PUAN_2_HALKA_KOMSU: 'komsu2', CARPAN_KURAKLIK: 'kurak',
            PUAN_ONLUK_KURAKLIK_BONUSU: 'onluk', PUAN_KINETIK_IVME_BONUSU: 'ivme',
            PUAN_GECIKMELI_TEKRAR: 'gecik', CARPAN_JOKER: 'joker',
            CEZA_OLU_SAYI_4: 'olu', OLUM_CEZASI_SINIRI: 'kurak_sinir',
            CEZA_CIFTE_TEKRAR: 'cifte', CEZA_DOYGUN_4: 'c4',
            CEZA_DOYGUN_8: 'c8', CEZA_DOYGUN_12: 'c12', CEZA_DOYGUN_16: 'c16',
            CEZA_IZOLASYON: 'izolasyon'
          };
          for (let k in best_config) {
            let el = document.getElementById('hm_' + idMap[k]);
            let sel = document.getElementById('ws-hm_' + idMap[k]);
