...
save_pa_new = '''H.savePuanAyarlari = function (btn) {
        ...
        // Absolutes
        const absIds = ['YUZDE_TUM_GECMIS', 'OLUM_CEZASI_SINIRI', 'NORM_TARIHSEL_CAP', 'NORM_GUNCELL_CAP', 'NORM_KURAKLIK_CAP'];
        absIds.forEach(id => {
          let el = document.getElementById('pset_' + id);
...
if save_pa_orig in html:
    ...
    print("Could not find H.savePuanAyarlari")


# 2. Update render openPuanAyarlari modal UI
# In the original, the modal is hardcoded in HTML as `<div id="puanAyarlariModal"` or something similar.
# Wait, `H.openPuanAyarlari` was replaced with `H.openModal` in my previous thought? NO, I assumed it was generated by JS, but actually it might be hardcoded HTML!

# Let's write a python script to see the hardcoded HTML of puan ayarları.
