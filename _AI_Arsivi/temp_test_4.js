    ...
    window.HavuzMotoru = {
        ...
        if(val('hm_komsu') !== undefined) this.mult_(window.HavuzMotoru.base_config.PUAN_1_HALKA_KOMSU * window.HavuzMotoru.mult_config.PUAN_1_HALKA_KOMSU) = val('hm_komsu');
        if(val('hm_komsu2') !== undefined) this.mult_(window.HavuzMotoru.base_config.PUAN_2_HALKA_KOMSU * window.HavuzMotoru.mult_config.PUAN_2_HALKA_KOMSU) = val('hm_komsu2');
        if(val('hm_kurak') !== undefined) this.mult_(window.HavuzMotoru.base_config.CARPAN_KURAKLIK * window.HavuzMotoru.mult_config.CARPAN_KURAKLIK) = val('hm_kurak');
        ...
        if(val('hm_ivme') !== undefined) this.mult_(window.HavuzMotoru.base_config.PUAN_KINETIK_IVME_BONUSU * window.HavuzMotoru.mult_config.PUAN_KINETIK_IVME_BONUSU) = val('hm_ivme');
        if(val('hm_gecik') !== undefined) this.mult_(window.HavuzMotoru.base_config.PUAN_GECIKMELI_TEKRAR * window.HavuzMotoru.mult_config.PUAN_GECIKMELI_TEKRAR) = val('hm_gecik');
        if(val('hm_olu') !== undefined) this.mult_(window.HavuzMotoru.base_config.CEZA_OLU_SAYI_4 * window.HavuzMotoru.mult_config.CEZA_OLU_SAYI_4) = val('hm_olu');
        if(val('hm_cifte') !== undefined) this.mult_(window.HavuzMotoru.base_config.CEZA_CIFTE_TEKRAR * window.HavuzMotoru.mult_config.CEZA_CIFTE_TEKRAR) = val('hm_cifte');
        if(val('hm_c4') !== undefined) this.mult_(window.HavuzMotoru.base_config.CEZA_DOYGUN_4 * window.HavuzMotoru.mult_config.CEZA_DOYGUN_4) = val('hm_c4');
        if(val('hm_c8') !== undefined) this.mult_(window.HavuzMotoru.base_config.CEZA_DOYGUN_8 * window.HavuzMotoru.mult_config.CEZA_DOYGUN_8) = val('hm_c8');
        if(val('hm_c12') !== undefined) this.mult_(window.HavuzMotoru.base_config.CEZA_DOYGUN_12 * window.HavuzMotoru.mult_config.CEZA_DOYGUN_12) = val('hm_c12');
        if(val('hm_c16') !== undefined) this.mult_(window.HavuzMotoru.base_config.CEZA_DOYGUN_16 * window.HavuzMotoru.mult_config.CEZA_DOYGUN_16) = val('hm_c16');
      ...
      puanlari_hesapla: function (df, maxN, joks = []) {
        ...
        let joker_carpani = (this.base_config.CARPAN_JOKER * this.mult_config.CARPAN_JOKER);
        ...
        for (let i = 1; i <= maxN; i++) {
          if (komsuSayaci_1[i] > 0)
            puanlar[i] += komsuSayaci_1[i] * (this.base_(window.HavuzMotoru.base_config.PUAN_1_HALKA_KOMSU * window.HavuzMotoru.mult_config.PUAN_1_HALKA_KOMSU) * this.mult_(window.HavuzMotoru.base_config.PUAN_1_HALKA_KOMSU * window.HavuzMotoru.mult_config.PUAN_1_HALKA_KOMSU));
          if (komsuSayaci_2[i] > 0)
            puanlar[i] += komsuSayaci_2[i] * (this.base_(window.HavuzMotoru.base_config.PUAN_2_HALKA_KOMSU * window.HavuzMotoru.mult_config.PUAN_2_HALKA_KOMSU) * this.mult_(window.HavuzMotoru.base_config.PUAN_2_HALKA_KOMSU * window.HavuzMotoru.mult_config.PUAN_2_HALKA_KOMSU));
        }
        ...
        for (let blok = 0; blok < onlukBlokSayisi; blok++) {
          ...
          if (!bloktan_cikan_var_mi) {
            for (let n = baslangic; n <= bitis; n++) {
              puanlar[n] += (this.base_config.PUAN_ONLUK_KURAKLIK_BONUSU * this.mult_config.PUAN_ONLUK_KURAKLIK_BONUSU);
            }
          }
        }
        ...
        for (let i = 1; i <= maxN; i++) {
          ...
          if (f5 >= 3) puanlar[i] += (this.base_(window.HavuzMotoru.base_config.CEZA_OLU_SAYI_4 * window.HavuzMotoru.mult_config.CEZA_OLU_SAYI_4) * this.mult_(window.HavuzMotoru.base_config.CEZA_OLU_SAYI_4 * window.HavuzMotoru.mult_config.CEZA_OLU_SAYI_4));
          if (f15 === 2 && f5 === 0) puanlar[i] += (this.base_(window.HavuzMotoru.base_config.PUAN_GECIKMELI_TEKRAR * window.HavuzMotoru.mult_config.PUAN_GECIKMELI_TEKRAR) * this.mult_(window.HavuzMotoru.base_config.PUAN_GECIKMELI_TEKRAR * window.HavuzMotoru.mult_config.PUAN_GECIKMELI_TEKRAR));
          ...
          if (count_3 >= 2) {
            puanlar[i] += (this.base_(window.HavuzMotoru.base_config.CEZA_DOYGUN_4 * window.HavuzMotoru.mult_config.CEZA_DOYGUN_4) * this.mult_(window.HavuzMotoru.base_config.CEZA_DOYGUN_4 * window.HavuzMotoru.mult_config.CEZA_DOYGUN_4));
          } else if (count_7 >= 3) {
            puanlar[i] += (this.base_(window.HavuzMotoru.base_config.CEZA_DOYGUN_8 * window.HavuzMotoru.mult_config.CEZA_DOYGUN_8) * this.mult_(window.HavuzMotoru.base_config.CEZA_DOYGUN_8 * window.HavuzMotoru.mult_config.CEZA_DOYGUN_8));
          } else if (count_11 >= 4) {
            puanlar[i] += (this.base_(window.HavuzMotoru.base_config.CEZA_DOYGUN_12 * window.HavuzMotoru.mult_config.CEZA_DOYGUN_12) * this.mult_(window.HavuzMotoru.base_config.CEZA_DOYGUN_12 * window.HavuzMotoru.mult_config.CEZA_DOYGUN_12));
          } else if (count_15 >= 5) {
            puanlar[i] += (this.base_(window.HavuzMotoru.base_config.CEZA_DOYGUN_16 * window.HavuzMotoru.mult_config.CEZA_DOYGUN_16) * this.mult_(window.HavuzMotoru.base_config.CEZA_DOYGUN_16 * window.HavuzMotoru.mult_config.CEZA_DOYGUN_16));
          }
      ...
      extractDetailsForUI: function (n, df, joks = []) {
        ...
        son_3_donem.forEach(cekilis => {
          if (cekilis && Array.isArray(cekilis)) {
            cekilis.forEach(sayi => {
              if (uiGridKomsulari(sayi, maxN).includes(n)) k6 += (window.HavuzMotoru.base_config.PUAN_1_HALKA_KOMSU * window.HavuzMotoru.mult_config.PUAN_1_HALKA_KOMSU);
              if (uiGrid2HalkaKomsulari(sayi, maxN).includes(n)) k7 += (window.HavuzMotoru.base_config.PUAN_2_HALKA_KOMSU * window.HavuzMotoru.mult_config.PUAN_2_HALKA_KOMSU);
            });
          }
        });
        ...
        if (count_3 < 2 && count_7 < 3 && f15 <= 1 && f3 >= 1) {
          k9 = (window.HavuzMotoru.base_config.PUAN_KINETIK_IVME_BONUSU * window.HavuzMotoru.mult_config.PUAN_KINETIK_IVME_BONUSU);
        }

        // K10- Gecikmeli Tekrar
        let k10 = 0;
        if (f15 === 2 && f5 === 0) {
          k10 = (window.HavuzMotoru.base_config.PUAN_GECIKMELI_TEKRAR * window.HavuzMotoru.mult_config.PUAN_GECIKMELI_TEKRAR);
        }
        ...
        let k12 = 0;
        if (f5 >= 3) {
          k12 = (window.HavuzMotoru.base_config.CEZA_OLU_SAYI_4 * window.HavuzMotoru.mult_config.CEZA_OLU_SAYI_4);
        }

        // K13- Çifte Tekrar Cezası
        let k13 = 0;
        if (df.length >= 2 && df[0] && df[1] && Array.isArray(df[0]) && Array.isArray(df[1]) && df[0].includes(n) && df[1].includes(n)) {
          k13 = (window.HavuzMotoru.base_config.CEZA_CIFTE_TEKRAR * window.HavuzMotoru.mult_config.CEZA_CIFTE_TEKRAR);
        }
        ...
        let doygunlukLabel = "";
        if (count_3 >= 2) {
          k14 = (window.HavuzMotoru.base_config.CEZA_DOYGUN_4 * window.HavuzMotoru.mult_config.CEZA_DOYGUN_4);
          doygunlukLabel = "";
        } else if (count_7 >= 3) {
          k14 = (window.HavuzMotoru.base_config.CEZA_DOYGUN_8 * window.HavuzMotoru.mult_config.CEZA_DOYGUN_8);
          doygunlukLabel = "";
        } else if (count_11 >= 4) {
          k14 = (window.HavuzMotoru.base_config.CEZA_DOYGUN_12 * window.HavuzMotoru.mult_config.CEZA_DOYGUN_12);
          doygunlukLabel = "";
        } else if (count_15 >= 5) {
          k14 = (window.HavuzMotoru.base_config.CEZA_DOYGUN_16 * window.HavuzMotoru.mult_config.CEZA_DOYGUN_16);
          doygunlukLabel = "";
        }
      ...
      runBacktest: function () {
        ...
        for (let i = testCount - 1; i >= 0; i--) {
          ...
          let gercek_joks = rawDraws.slice(i + 1).map(d => d.joker || null);
          let hedef_cekilis = rawDraws[i].nums.slice(0, 6);
          let rawD = rawDraws[i].date || `Geçmiş -${i + 1}`; let dps = rawD.split('-'); let hedef_tarih = dps.length === 3 ? dps[2]+'-'+dps[1]+'-'+dps[0] : rawD;
        ...
        for (let k = 6; k >= 0; k--) {
          ...
          if (items && items.length > 0) {
            ...
            <span><b style="font-size:18px;">${k} Bilen</b> Sayısı: <b style="color:#fff;">${items.length}</b></span>
